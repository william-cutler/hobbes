...
class Stage1ModelV2(pl.LightningModule):
	def __init__(self):
		...
		# input: [N, 3, 200, 200]
		# output: [N, 64]
		self.static_vision_encoder = VisionNetwork(200, 200, 'LeakyReLU', 0.0, True, 64, 3)

		# for 'rgb_gripper' camera
		# input: [N, 3, 84, 84]
		# output: [N, 32]
		self.gripper_vision_encoder = VisionNetwork(84, 84, 'LeakyReLU', 0.0, True, 32, 3)

		# for proprioceptive information ('robot_obs')
		self.robot_encoder = nn.Identity()

		self.action_encoder = nn.TransformerEncoder(d_model=111, activation='LeakyReLU')
