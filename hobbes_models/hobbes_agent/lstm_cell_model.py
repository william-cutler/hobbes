...
class HobbesLSTMCell(pl.LightningModule):
    def __init__(
        self, img_embed_dim=64, encoder_hidden_dim=64, decoder_hidden_dim=64, dropout=0.1,
    ):
        super().__init__()
        
        # encoder for images (batch, 3, 200, 200) -> (batch, embed_dim(64))
        self.vision_encoder = VisionNetwork(200, 200, 'LeakyReLU', 0.0, True, img_embed_dim, 3)

        self.dropout = nn.Dropout(p=dropout)

        # # LSTM https://pytorch.org/docs/stable/generated/torch.nn.LSTM.html
        # # (batch, img_embed_dim(64)) -> (batch, encoder_hidden_dim(64))
        # self.lstm_encoder = nn.LSTM(input_size=img_embed_dim,
        #                             hidden_size=encoder_hidden_dim,
        #                             num_layers=1,
        #                             batch_first=True)

        # # (batch, img_embed_dim(64) + encoder_hidden_dim(64)) -> (batch, decoder_hidden_dim(64))
        # self.lstm_decoder = nn.LSTM(input_size=img_embed_dim + encoder_hidden_dim,
    ...
    def forward(self, demonstration_images, demonstration_images_num, runtime_image):
        ...
        demonstration_x = self.vision_encoder(demonstration_images)
        ...
        runtime_x = torch.cat(
            [runtime_x, encoder_final_hidden]
        )
        # (batch, 128)
        ...
        output, _ = self.lstm_decoder(
            ...
            h_0=encoder_final_hidden.unsqueeze(0),  # hidden
            c_0=torch.zeros_like(encoder_final_hidden).unsqueeze(0),  # cell
        )
